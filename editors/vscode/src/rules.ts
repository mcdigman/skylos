...
const RULES: Record<string, RuleMeta> = {
  ...
  "SKY-D282": { name: "Unverified webhook handler", severity: "HIGH", category: "security", description: "Webhook handler processes inbound provider events without obvious signature verification.", cwe: "CWE-347", fix: "Verify provider signatures before parsing or trusting the webhook body.", language: "typescript" },
  ...
  "SKY-P403": { name: "Quadratic lookup", severity: "MEDIUM", category: "quality", description: "A loop repeats a lookup that scales with the input: either a nested loop matching values across two collections, or a list membership test re-scanned every iteration.", fix: "Build a dict or set from the inner collection once before the loop, then look the match up directly." },

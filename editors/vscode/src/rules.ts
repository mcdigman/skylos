...
const RULES: Record<string, RuleMeta> = {
  ...
  "SKY-P403": { name: "Quadratic lookup", severity: "LOW", category: "quality", description: "A loop repeats a lookup that scales with the input: either a nested loop matching values across two collections, or a list membership test re-scanned every iteration.", fix: "Build a dict or set from the inner collection once before the loop, then look the match up directly." },
